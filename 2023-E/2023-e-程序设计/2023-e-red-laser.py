...
class UartDriver:
    ...
    def main_loop_handler(self):
        if self.uart.any():  # 如果有数据可读
            data = self.uart.read(1)  # 逐字节读取数据
            ...
            if data == '@':  # 如果是命令开始标志
                ...
                self.execute_command(self.cmd)  # 执行命令
                self.cmd = ""  # 清空命令缓存
            else:
                # 在命令捕获状态下，拼接字符
                if self.start_flag:
    ...
    def execute_command(self, command):
        print("接收到命令:", command)  # 打印接收到的命令
        # 根据命令执行对应的任务
        if command == "TASK1_1":
            self.task1_1()
        elif command == "TASK1_2":
            self.task1_2()
        elif command == "TASK1_3":
            self.task1_3()
