...
static void setup_task5(void) {
    car_path_init();                                           // 初始化路径规划
    car_add_float(&car.track_speed, C22_BASE_SPEED - 5);      // 设置循迹速度为基准速度-5（35）
...
static void setup_task6(void) {
    car_path_init();                                           // 初始化路径规划
    car_add_float(&car.track_speed, C22_BASE_SPEED);          // 设置循迹速度为基准速度（40）
    car_add_bool(&is_outer_track, true);                      // 设置标志位：走外圈模式
...
void run_task06_cb(void *arg) {
		run_task("Running Task 02", &task_running_flag, setup_task6);
}
